...
*/
vendor('h2o/h2o');
if(!defined('H2O_COMPILE_PATH')) {
	define('H2O_COMPILE_PATH', dirname(dirname(__FILE__)).DS.'tmp'.DS.'compile');
	if(!file_exists(H2O_COMPILE_PATH)){
		@mkdir(H2O_COMPILE_PATH);
	}
}
h2o::load_plugin('cakephp');

class H2oView extends View {
	var $h2oAutoRender = false;
	var $h2oEnableCache = false;
	var $ext = '.html';
