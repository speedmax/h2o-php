...
class Describe_Parser_Patterns extends SimpleSpec {
    ...
    function should_match_basic_operators () {
        ...
        expects($this->_regex['whitespace'])->should_match(' ');
        expects($this->_regex['whitespace'])->should_match('         ');
    }

    function should_match_numeric_values() {
        expects($this->_regex['number'])->should_match('0.555');
        expects($this->_regex['number'])->should_match('2000');
    }
    
    function should_match_string() {
        # Double quote
        expects($this->_regex['string'])->should_match('"this is a string"');
        expects($this->_regex['string'])->should_match('"She has \"The thing\""');
        
        # Single Quote
        expects($this->_regex['string'])->should_match("'the doctor is good'");
        expects($this->_regex['string'])->should_match("'She can\'t do it'");
    }
    
    function shuold_match_i18n_string() {
        expects($this->_regex['i18n_string'])->should_match("_('hello world')");
    }
    
    function should_match_variable_name() {
        expects($this->_regex['name'])->should_match('something');
        expects($this->_regex['name'])->should_match('somet-hing');
        expects('something.something')->should_match($this->_regex['name']);
    }
    
    function should_match_named_arguments() {
        expects($this->_regex['named_arg'])->should_match("name: 'peter'");
        expects($this->_regex['named_arg'])->should_match("name: variable");
        expects($this->_regex['named_arg'])->should_match("price: 0.55 ");
        expects($this->_regex['named_arg'])->should_match("age: 199 ");
        expects($this->_regex['named_arg'])->should_match("alt: _('image alt tag')");
    }
    ...
    function should_match_operators() {
        ...
        foreach($operators+$logics as $op) {
            expects($op)->should_match($this->_regex['operator']);
        }
    }
}
...
class Describe_Argument_Lexer extends SimpleSpec {
    ...
    private function parse($string) {
        return h2o_Argument::parse($string);
    }
}
